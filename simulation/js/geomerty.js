...
(function (Chemistry) {
    ...
    class Inter_Rings extends Geometry {
        constructor(n, d, color, stpt, canvas) {
            super();
            this.all_left_readings = [];
            this.all_right_readings = [];
        ...
        draw() {
            ...
            for (let i = 0; i < this.n; i++) {
                //for recording the left values
                let msr = (this.stpt.x + (this.d[i] * this.multiplier / 2) - this.center_x) / (inter_rings.multiplier / 2);
                msr = msr / 2;
                let msr_val = Math.floor(parseFloat((msr / 2).toFixed(3)) / (0.05));
                let vsr_val = Math.floor((parseFloat((msr / 2).toFixed(3)) - (msr_val * 0.05)) * 1000);
                this.all_left_readings.push([msr_val, vsr_val]);
                //for recording the right values
                let msr2 = (this.center_x - this.stpt.x + (this.d[i] * this.multiplier / 2)) / (inter_rings.multiplier / 2);
                msr2 = msr2 / 2;
                let msr2_val = Math.floor(parseFloat((msr2 / 2).toFixed(3)) / (0.05));
                let vsr2_val = Math.floor((parseFloat((msr2 / 2).toFixed(3)) - (msr2_val * 0.05)) * 1000);
                this.all_right_readings.push([msr2_val, vsr2_val]);
                ...
                // console.log(this.color);
                this.context.beginPath();
                this.context.arc(this.stpt.x * lscale, this.stpt.y * lscale, this.d[i] * this.multiplier * lscale / 3, 0, 2 * Math.PI, false);
                // console.log(this.d[i]*multiplier*lscale/2);
                if (i == 0) {
                    this.context.lineWidth = this.d[i] * this.multiplier * lscale / 3;
                    context.fillStyle = "red";
                    this.context.fill();
                }
                else {
                    this.context.lineWidth = (20) / (i + 1);
                }
                this.context.stroke();
                this.context.strokeStyle = 'black';
            }
    ...
    Chemistry.Inter_Rings = Inter_Rings;
